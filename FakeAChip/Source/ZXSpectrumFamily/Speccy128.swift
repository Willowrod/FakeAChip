...
class Speccy128: Speccy {
    ...
    func swapRom(rom: Int){
        pauseProcessor = true
        print ("Swapping ROM to Rom \(rom)")
        switch rom {
        case 0:
            print ("128k Rom in use")
            memory[0].removeAll()
            memory[0].append(contentsOf: rom128k)
            currentRom = .Sinclair_Spectrum_128K
        case 1:
            print ("48k Rom in use")
            memory[0].removeAll()
    ...
    func swapRam(bank: Int){
        print("Swapping to RAM Bank \(bank) - 1")
    ...
    func swapRam(bank: Int, contents:[UInt8]){
        print("Swapping to RAM Bank \(bank) - 2")
        if (bank != currentSwapBank){
            pauseProcessor = true
            ramBanks[currentSwapBank].removeAll()
            ramBanks[currentSwapBank].append(contentsOf: memory[3])
  ...
  //      print("Loading location \(UInt16(location).hex()) with value \(value.hex())")
        if location < 0x4000 {
            print("Cannot write to ROM")
            return
        }
